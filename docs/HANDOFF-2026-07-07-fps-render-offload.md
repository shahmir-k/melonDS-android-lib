...
2. **Emulation frameskip** — skip whole EMULATED frames (not just raster; AGGRESSIVE_SKIP only skips
   rasterization today). Potentially ~2× throughput. RISKY (input lag, physics desync, softlock) —
   gate hard on playable. Core-side, headless-measurable. Biggest single core lever.
